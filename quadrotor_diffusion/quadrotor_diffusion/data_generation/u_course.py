...
def create_polynomial_trajectories(course: list[np.array]) -> list[PolynomialTrajectory]:
    ...
    # Add first gate
    is_close = np.linalg.norm((course[1] - course[0])[:2]) < 1.0
    time_bound = [0.5, 2.5] if is_close else [2.0, 3.0]
    vel_bound = [0.8, 1.2] if is_close else [1.2, 1.4]
    trajectories = add_next_gate(course, 1, vel_bound, time_bound, [init_trajectory], [
    ...
    trajectories = add_next_gate(course, 2, [0.2, 1.5], [0.5, 2.0], trajectories,
                                 [BoundarySamplingStrategy.NEAR_ZERO, BoundarySamplingStrategy.NEW_VELOCITY_ZERO_ACCEL])
    ...
    trajectories = add_next_gate(course, 3, [0.8, 1.5], [1.8, 2.5], trajectories,
                                 [BoundarySamplingStrategy.NEW_VELOCITY_ZERO_ACCEL, BoundarySamplingStrategy.NEW_NORMAL_VELOCITY_AND_ACCEL])
    if len(trajectories) > MAX_TRAJECTORIES_TO_EXAMINE_PER_STEP:
        random.shuffle(trajectories)
        trajectories = trajectories[:MAX_TRAJECTORIES_TO_EXAMINE_PER_STEP]
...
def generate_u() -> tuple[list[np.array], list[PolynomialTrajectory]]:
    """
    Creates a u course
    ...
    gate_2 = np.array([
        ...
        random.choice([0.3, 0.525]),
        np.random.uniform(0.0, 0.8 * -np.pi / 2)
    ])
    ...
    gate_3 = np.array([
        ...
        np.random.uniform(0.4, 1.2),
        random.choice([0.3, 0.525]),
        np.random.uniform(0.8 * np.pi / 2, 0) + np.pi
    ])
    ...
    gate_4 = np.array([
        ...
        np.random.uniform(-1.0, -0.2),
        random.choice([0.3, 0.525]),
        np.random.uniform(0.0, -0.5) + np.pi
    ])
